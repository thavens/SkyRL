...
def sweep(
    ...
    def run(label, backend_cfg, seq_len, batch_seqs, adapters, steps, warmup, timeout):
        argv = [
            ...
            f"{REPO}/tools/bench_trainer_configs.py",
            "--base-model",
            MODEL_DIR,
            "--backend-config",
            json.dumps(backend_cfg),
            "--label",
            label,
            "--seq-len",
            str(seq_len),
        ...
        try:
            proc = subprocess.run(argv, env=env, cwd=REPO, timeout=timeout, capture_output=True, text=True)
    ...
    if not skip_smoke:
        smoke = run(
            "smoke", cfg(max_lora_adapters=2), seq_len=256, batch_seqs=2, adapters=1, steps=1, warmup=1, timeout=1800
        )
    ...
    for label, backend_cfg in todo:
        results.append(run(label, backend_cfg, seq_len, batch_seqs, ADAPTERS, steps=3, warmup=1, timeout=2700))
