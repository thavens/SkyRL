...
VLLM_LORA_RESOLVER_CACHE_DIR=/dev/shm/$USER/qwen35_9b_lora_models \
...
| 9B, 2 LoRAs | 0.90 | Same as 9B but `"max_lora_adapters":2,"max_lora_rank":32`. At rank 64 the second slot's ~1.7 GiB/GPU pushes the 4096-token bucket out of budget (16.71 GiB temp arena OOM, verified 2026-08-10); rank 32 restores it. Rank 64 with 2 slots trains only up to 3072. |
...
- **Sampler LoRA adapters** go on tmpfs (`/dev/shm/$USER/...`), not `STATE_DIR`. vLLM never unloads them; `rm -rf` when bouncing vLLM. The `VLLM_PLUGIN_LORA_RESOLVERS=lora_filesystem_resolver` + `VLLM_LORA_RESOLVER_CACHE_DIR=<that dir>` pair on the vLLM launch is what lets vLLM load adapters by name on demand — without both, every LoRA sample 404s.
...
| Variable | Why |
|----------|-----|
| `XLA_FLAGS=--xla_gpu_unsupported_use_all_reduce_one_shot_kernel=false` | jax 0.10.2's one-shot all-reduce kernel aborts every TP=2 `forward_backward` at execution time with `INVALID_ARGUMENT: Unsupported AllReduce kernel` on this box (sm_89, no NVLink). Compiles fine; fails only when run. |
...
- **Sampler `--max-model-len` must exceed the trainer cap** (we use 5120 vs 4096): probing prompt logprobs on a max-length trajectory needs prompt + 1 token of context, so equal caps make every end-of-trajectory probe a 400.
...
Each slot costs 1.68 GiB/GPU (stacked LoRA weights + fp32 grad/mu/nu at rank 64, 9B TP=2); slot cost scales with `max_lora_rank`, so rank 32 halves it. Set it to the number of concurrent training clients you need — but on 32 GB cards the 4096-token `forward_backward` needs a 16.71 GiB temp arena, so every extra rank-64 slot beyond the first costs one bucket of max trainable sequence length (2 slots at rank 64 → 3072 cap). Prefer lowering `max_lora_rank` over losing sequence length.

Dead clients pin their slot until session expiry (~6 min). A client that retries `create_lora_training_client` in a loop against a full server can livelock with expiry; retry with ≥30 s backoff.
...
The forwarder now unloads a model's previous sampler adapter once the replacement has served a request, so vLLM should hold only the current adapter per model. Adapters piling up (25 observed in one run before this) means the unload is failing — check the server log for `Failed to unload stale LoRA adapter`.

The staging directories on disk are not reaped, so `rm -rf /dev/shm/$USER/qwen35_${TAG}_lora_models` is still the way to reclaim that space (vLLM must be stopped first).

### Samples fail with a bare 400 from `/v1/completions`

vLLM rejected the forwarded request; the reason is in the vLLM log (and, since 2026-08-10, in the error detail). Known triggers: `topk_prompt_logprobs` above vLLM's `--max-logprobs` (default 20; we launch with 128), `temperature=0` with `num_samples>1`, prompt+max_tokens over `--max-model-len`.

### vLLM engine dies with CUDA OOM on prompt-logprob requests

`prompt_logprobs` materializes full-vocab fp32 logits for every prompt position in the prefill chunk: chunk 2048 × 248,320 vocab × 4 B = 1.89 GiB in one transient allocation, which kills the engine (and every in-flight request) at `--gpu-memory-utilization 0.90`. This is why the launch sets `--max-num-batched-tokens 1024` (halves the transient) and `0.85` utilization (~1.6 GiB headroom). If it recurs, lower to 512/0.85. Decode throughput is unaffected; KV shrinks ~795k → ~700k tokens.
...
### 9B, 2 LoRA slots (rank 32), measured 2026-08-10

Synchronous RL loop per client (sample 8 prompts × 8 samples @ 384 new tokens, then train 32 seqs avg ~2.4k tokens): trainer executes `forward_backward` at ~1,500 tok/s per request and requests from the two clients serialize, giving ~1,480 tok/s aggregate when busy. The synchronous loop only keeps the trainer 65% busy (measured 62% GPU util) because each client's trainer work idles during its own sampling phase — aggregate ~970 tok/s trained.

- **Biggest lever: async pipelining in the client** (train batch *i* while sampling batch *i+1*): recovers the idle duty cycle, ~+50% end-to-end throughput. Server-side request coalescing would not help: `train_micro_batch_size=1` makes compute per-sequence, so batching two clients' requests saves only dispatch overhead.
- **The sampler is nowhere near saturated** (25% GPU util at 128 concurrent seqs, ~2,150 tok/s aggregate generation): raising `num_samples` or prompt concurrency is close to free.
- Sampler weight publish (save_weights_and_get_sampling_client) takes 2–4 s; per-execution trainer efficiency ≈ 42 TFLOP/s/GPU incl. remat recompute — near this card's practical roofline, so don't chase kernel-level wins.
